//! w2d2 — веб-чат с сохранением контекста. Здесь только HTTP: маршруты,
//! общее состояние и переклад событий агента в SSE. История лежит в файлах
//! (`store.rs`), формат API провайдеров — в `agent.rs`.
...
use agent::{Agent, Event, Provider, Settings, PERSONAS};
...
async fn start() -> Result<(), String> {
    println!("AI Advent · w2d2 — агент с памятью на диске");
