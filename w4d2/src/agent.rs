...
На этапе выполнения доступен MCP-инструмент search_repositories: ищи публичные GitHub-проекты по запросу пользователя. Один поиск за ход. Формируй краткий запрос с нужными фильтрами языка и темы. Exa пока показывает только каталог.
...
fn requested_tool(message: &Value) -> Result<Option<(String, Value)>, String> {
    ...
    if calls.len() != 1 { return Err("За один ход разрешён один поиск GitHub. Уточни запрос.".into()); }
    let call = &calls[0];
    if call["type"] != "function" || call["function"]["name"] != crate::github::TOOL {
    ...
    Ok(Some((id.to_string(), arguments)))
}
...
impl Agent {
    ...
    /// Модель получает схему из нашего MCP-сервера и сама решает, нужен ли поиск.
    ...
    async fn github_draft(
        ...
        let mut client = crate::mcp::connect_github().await?;
        let tools = tokio::time::timeout(Duration::from_secs(5), client.list_all_tools()).await
            .map_err(|_| "Тайм-аут каталога GitHub MCP")?.map_err(|e| e.to_string())?;
        let tool = tools.into_iter().find(|tool| tool.name == crate::github::TOOL)
            .ok_or("MCP-сервер не зарегистрировал search_repositories")?;
        ...
        let (id, arguments) = requested.unwrap();
        let mut trace = crate::mcp::ToolTrace {
            name: crate::github::TOOL.into(), arguments: arguments.clone(), result: None,
        };
        let _ = tx.send(Event::Tool(trace.clone())).await;
        let params = CallToolRequestParams::new(crate::github::TOOL)
...
mod tests {
    ...
    fn native_tool_exchange_preserves_id_result_and_reasoning() {
        ...
        let (id, args) = requested_tool(&raw).unwrap().unwrap();
        ...
        assert!(requested_tool(&unknown).is_err());
        let mut multiple = raw.clone();
        multiple["tool_calls"].as_array_mut().unwrap().push(raw["tool_calls"][0].clone());
        assert!(requested_tool(&multiple).is_err());
        assert!(requested_tool(&json!({"content":"ordinary answer"})).unwrap().is_none());
    }
