//! w4d2 — веб-чат с явной моделью памяти агента, инструкциями пользователя,
...
async fn start() -> Result<(), String> {
    ...
    if mode.as_deref() == Some("--github-mcp-server") {
        return github::serve().await;
    }
    ...
    println!("AI Advent · w4d2 — исследователь: поиск GitHub-проектов через MCP");
