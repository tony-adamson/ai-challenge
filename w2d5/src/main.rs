//! w2d5 — веб-чат со стратегиями контекста. Здесь только HTTP: маршруты,
//! общее состояние и переклад событий агента в SSE. История и ветки лежат в
//! файлах (`store.rs`), формат API провайдеров, стратегии и вся арифметика
//! токенов — в `agent.rs`.
...
use agent::{is_context_overflow, Agent, Event, Provider, Settings, Strategy, PERSONAS};
...
async fn start() -> Result<(), String> {
    println!("AI Advent · w2d5 — агент со стратегиями контекста");
    ...
    let router = Router::new()
        ...
        .route("/api/chats/{id}/facts", delete(clear_facts))
        .route("/api/chats/{id}/facts/{index}", delete(drop_fact))
...
/// Ручная правка памяти: факт можно выбросить, если модель запомнила лишнее.
/// Редактирования значений нет — для этого проще сказать модели новое.
async fn drop_fact(
    State(app): State<Arc<App>>,
    Path((id, index)): Path<(String, usize)>,
) -> Response {
    let Some(mut chat) = app.store.load(&id) else { return not_found() };
    if index >= chat.facts.len() {
        return (StatusCode::BAD_REQUEST, "такого факта нет").into_response();
    }
    chat.facts.remove(index);
    save_facts(&app, &chat)
}

async fn clear_facts(State(app): State<Arc<App>>, Path(id): Path<String>) -> Response {
    let Some(mut chat) = app.store.load(&id) else { return not_found() };
    chat.facts.clear();
    save_facts(&app, &chat)
}

/// Накопительные счётчики фактов при правке не трогаем: обновления уже были
/// оплачены, и обнулять их из-за выброшенной строки было бы враньём.
fn save_facts(app: &App, chat: &Chat) -> Response {
    match app.store.save(chat) {
        Ok(()) => Json(json!({ "facts": chat.facts })).into_response(),
        Err(error) => (StatusCode::INTERNAL_SERVER_ERROR, error).into_response(),
    }
}
...
async fn ask(
    ...
    tokio::spawn(async move {
        ...
        // Факты обновляются после ответа, а не до: так в память попадают и
        // решения самой модели, а человек не ждёт лишнего запроса перед
        // стримом. Один ход — одно обновление; осечка памяти уже отданный
        // ответ не отменяет.
        if answered && chat.settings.strategy == Strategy::Facts {
            match app.agent.update_facts(&mut chat).await {
                Ok(info) => match app.store.save(&chat) {
                    Ok(()) => {
                        let _ = tx.send(Event::Facts(info)).await;
                    }
                    Err(error) => eprintln!("факты не сохранены: {error}"),
                },
                Err(error) => {
                    let _ = tx.send(Event::FactsError(error)).await;
                }
            }
        }
    ...
    let stream = ReceiverStream::new(rx).map(|event| {
        let (name, data) = match event {
            ...
            // Факты, наоборот, приезжают после `done`: они собираются по уже
            // готовому ответу. Поток закрывается только после них.
            Event::Facts(info) => ("facts", json!(info)),
            Event::FactsError(error) => ("facts_error", json!({ "error": error })),
