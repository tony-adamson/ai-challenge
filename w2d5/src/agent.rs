...
/// Стратегия контекста: что именно уезжает провайдеру из истории чата.
/// Полная история ничего не теряет и ничего не экономит, остальные три —
/// разные способы разменять детали на токены.
...
pub enum Strategy {
    ...
    Window,
    /// Блок фактов плюс последние `keep_last` сообщений.
    Facts,
}

impl Strategy {
    pub const ALL: [Strategy; 4] =
        [Strategy::Full, Strategy::Summary, Strategy::Window, Strategy::Facts];
...
pub struct Settings {
    ...
    /// Как собирается контекст запроса. Смена стратегии ничего из файла чата
    /// не удаляет: и пересказ, и факты остаются на месте, их просто перестают
    /// подставлять.
    pub strategy: Strategy,
    /// Сколько последних сообщений всегда идут в запрос как есть. Общее число
    /// для `summary`, `window` и `facts`.
    pub keep_last: usize,
...
pub struct Metrics {
    ...
    /// Сколько реплик истории ушло в этот запрос (без системного промпта и
    /// без служебного сообщения с summary).
    #[serde(default)]
    ...
    /// То же самое для блока фактов. Поле отдельное, а не общее «служебный
    /// блок»: в строке под ответом видно, чем именно заменена история.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub facts_tokens_estimate: Option<u64>,
...
pub enum Event {
    ...
    SummaryError(String),
    /// Факты обновлены: новый список и что он стоил. Уходит после `done` —
    /// факты собираются по уже готовому ответу.
    Facts(FactsInfo),
    /// Обновить факты не вышло. Ответ пользователю к этому моменту уже ушёл,
    /// список фактов остаётся прежним.
    FactsError(String),
    Error(String),
}
...
/// Результат одного обновления фактов — по той же форме, что и `SummaryInfo`.
#[derive(Debug, Clone, Serialize)]
...
/// Служебное сообщение с фактами. Роль та же `system` и по той же причине,
/// что у пересказа: это не чья-то реплика, а заметка о том, что известно про
/// разговор. Пустой список блока не даёт — пустую шапку модели слать незачем.
fn facts_message(facts: &[Fact]) -> Option<Message> {
    if facts.is_empty() {
        return None;
    }
    let lines: Vec<String> =
        facts.iter().map(|f| format!("- {}: {}", f.key, f.value)).collect();
    Some(Message::new("system", format!("Известные факты о диалоге:\n{}", lines.join("\n"))))
}

/// Что уходит провайдеру — единственное место сборки запроса, по нему же
/// считаются `sent_chars` для калибровки. Разница между стратегиями ровно
/// здесь и живёт:
///
/// - `full` — системный промпт и вся история;
/// - `summary` — промпт, пересказ и всё непокрытое им (между суммаризациями
///   хвост длиннее `keep_last`, так и задумано);
/// - `window` — промпт и последние `keep_last` **элементов `messages`**:
///   граница считается по индексам массива, а фильтр `goes_to_api` работает
///   уже после неё, как и у покрытия пересказа. Поэтому отклонённый запрос
///   место в окне занимает, но провайдеру не уходит, и разделитель в ленте
///   встаёт ровно по этой границе;
/// - `facts` — то же окно плюс блок фактов вторым системным сообщением.
///
/// Записи `error` отбрасываются здесь же: в API такой роли нет.
pub fn request_messages(chat: &Chat, settings: &Settings, system_prompt: &str) -> Vec<Message> {
    ...
    let tail = match settings.strategy {
        ...
        Strategy::Summary if uses_summary(chat, settings) => {
            ...
            out.push(summary_message(chat.summary.as_deref().unwrap_or(""), chat.summary_covers));
        ...
        Strategy::Facts => {
            if let Some(block) = facts_message(&chat.facts) {
                out.push(block);
            }
            &chat.messages[chat.window_start(settings.keep_last)..]
        }
    };
    out.extend(tail.iter().filter(|m| goes_to_api(m)).cloned());
    out
}
...
/// Промпт хранителя фактов. Тот же приём, что у суммаризатора: отдельный
/// дешёвый не-стримовый вызов, только просим не прозу, а JSON — разобрать
/// объект надёжнее, чем список строк. Просим не весь список, а дельту:
/// повторять всю память каждый ход модель не справляется, и молча
/// пропущенный факт раньше означал его потерю.
const FACTS_PROMPT: &str = "Ты ведёшь память о диалоге в виде фактов. На входе текущий список фактов и последняя пара реплик. Верни ТОЛЬКО JSON-объект вида {\"ключ\": \"значение\"} с фактами, которые появились или изменились в последней паре реплик: цель, ограничения, предпочтения, решения, договорённости, числа и имена. Неизменившиеся факты повторять не нужно. Чтобы удалить устаревший факт, верни его ключ с пустой строкой. Если новых фактов нет — верни {}. Не добавляй ничего, чего не было в диалоге. Ключи короткие, на русском, значения не длиннее 120 символов. Без пояснений и без markdown.";
const FACTS_MAX_TOKENS: u32 = 400;
/// Сколько символов реплики отдаём хранителю фактов — по той же причине, что
/// и суммаризатору: «Длинный текст» переполнил бы уже его самого.
const FACTS_SOURCE_LIMIT: usize = 2000;
/// Потолок памяти. Сорок фактов — это уже не выжимка, а вторая история.
const FACTS_LIMIT: usize = 40;

/// Текущие факты в том виде, в каком модель должна их вернуть.
...
/// Вход хранителя фактов: что известно сейчас и последняя пара реплик.
fn facts_source(facts: &[Fact], user: &str, assistant: &str) -> String {
    format!(
        "Текущие факты:\n{}\n\nЧеловек: {}\n\nАссистент: {}",
        facts_json(facts),
        head(user, FACTS_SOURCE_LIMIT),
        head(assistant, FACTS_SOURCE_LIMIT)
    )
}
...
fn facts_body(
    ...
    let mut body = json!({
        ...
        "messages": [
            ...
            { "role": "user", "content": facts_source(facts, user, assistant) },
        ],
        "stream": false,
        "temperature": 0.2,
    });
    ...
    match provider {
        Provider::Cerebras => {
            map.insert("max_completion_tokens".to_string(), json!(FACTS_MAX_TOKENS));
            map.insert("reasoning_effort".to_string(), json!("none"));
        }
        Provider::DeepSeek => {
            map.insert("max_tokens".to_string(), json!(FACTS_MAX_TOKENS));
            map.insert("thinking".to_string(), json!({ "type": "disabled" }));
        }
        Provider::OpenRouter => {
            map.insert("max_tokens".to_string(), json!(FACTS_MAX_TOKENS));
...
/// Ключи фактов сравниваются без учёта регистра и пробелов по краям: «Бюджет»
/// и «бюджет » — один и тот же факт.
fn same_key(left: &str, right: &str) -> bool {
    left.trim().to_lowercase() == right.trim().to_lowercase()
}

/// Разбор ответа хранителя фактов. Модель иногда отвечает числом вместо
/// строки — приводим к тексту, а не выбрасываем факт. Всё, что не разобралось
/// как объект, — ошибка: лучше оставить прежние факты, чем стереть их.
/// Пустое значение здесь сохраняется: для слияния это признак «забыть факт».
/// Потолок памяти тут не применяется — список приходит дельтой, режет его
/// `merge_facts` уже после склейки.
fn parse_facts(raw: &str) -> Result<Vec<Fact>, String> {
    let text = strip_code_fence(raw);
    let value: Value = serde_json::from_str(text)
        .map_err(|_| format!("факты пришли не в JSON: {}", truncate(text, 120)))?;
    let object = value.as_object().ok_or("факты пришли не объектом")?;
    ...
    for (key, value) in object {
        ...
        if facts.iter().any(|f| same_key(&f.key, key)) {
            continue;
        }
        facts.push(Fact {
            key: key.to_string(),
...
/// Склейка присланной дельты с текущей памятью. Известный ключ обновляется на
...
/// Последняя пара реплик: по ней и обновляются факты. Нет ответа ассистента —
/// обновлять нечего.
...
impl Agent {
    ...
    pub async fn ask(
        ...
        let messages = request_messages(chat, &chat.settings, &chat.settings.system_prompt);
        // Символы считаем по тому, что реально уходит (вместе со служебным
        // блоком): по ним и по `prompt_tokens` из ответа калибруется «сколько
        // символов в токене» для оценки на клиенте.
        let sent_chars = sent_chars(&messages);
        let strategy = chat.settings.strategy;
        // Первое сообщение — системный промпт, второе (у summary и facts) —
        // служебный блок: реплики разговора начинаются после них.
        let block = match strategy {
            Strategy::Summary => uses_summary(chat, &chat.settings),
            Strategy::Facts => !chat.facts.is_empty(),
            _ => false,
        };
        let sent_messages = messages.len() - 1 - usize::from(block);
        let block_estimate = block.then(|| {
            estimate_tokens(messages[1].content.chars().count(), chat.chars_per_token_or_default())
        });
        // «Без стратегии было бы» считаем везде, где стратегия вообще что-то
        // меняет: у summary — только после первого сворачивания, у окна и
        // фактов — всегда, они режут историю с первого же хода.
        let applied = match strategy {
            Strategy::Full => false,
            Strategy::Summary => block,
            Strategy::Window | Strategy::Facts => true,
        };
        let full_estimate =
        ...
        match outcome {
            Ok((answer, mut metrics)) => {
                ...
                metrics.summary_tokens_estimate =
                    (strategy == Strategy::Summary).then_some(block_estimate).flatten();
                metrics.facts_tokens_estimate =
                    (strategy == Strategy::Facts).then_some(block_estimate).flatten();
    ...
    /// Обновить факты по последней паре реплик — одним не-стримовым запросом к
    ...
    /// лишнего вызова. Один ход — один такой запрос.
    ///
    /// Модель возвращает дельту — появившееся и изменившееся, — и она
    /// **сливается** с прежним списком (`merge_facts`): пустой объект значит
    /// «без изменений», а не «очистить». Требовать повторения всей памяти
    /// каждый ход оказалось нереалистично: на живом прогоне модель трижды
    /// недосчиталась части списка, и замена целиком эти факты стёрла.
    /// Любая осечка — ошибка текстом и прежние факты на месте: терять память
    /// из-за одного кривого ответа незачем.
    pub async fn update_facts(&self, chat: &mut Chat) -> Result<FactsInfo, String> {
        let provider = chat.settings.provider()?;
        let key = self.key(provider)?.clone();
        let (user, assistant) = last_exchange(chat).ok_or("обновлять факты не по чему")?;
        let body = facts_body(provider, &chat.settings.model, &chat.facts, &user, &assistant);
        ...
        let incoming = parse_facts(&text)?;

        merge_facts(&mut chat.facts, incoming);
        let facts = chat.facts.clone();
        chat.facts_prompt_tokens += usage.prompt_tokens;
        chat.facts_completion_tokens += usage.completion_tokens;
        let cost = usage
            .api_cost_usd
            .or_else(|| cost(provider, &chat.settings.model, usage))
            .unwrap_or(0.0);
        chat.facts_cost += cost;
...
mod tests {
    ...
    fn the_full_strategy_sends_the_whole_history() {
        ...
        let messages = request_messages(&chat, &settings, "промпт");
        ...
        assert_eq!(request_messages(&chat, &waiting, "промпт").len(), 6);
        let empty = Chat { summary: Some("   ".to_string()), summary_covers: 3, ..chat.clone() };
        assert_eq!(request_messages(&empty, &waiting, "промпт").len(), 6, "пустой пересказ не в счёт");
    }
    ...
    fn a_summary_replaces_the_covered_head_of_the_history() {
        ...
        assert_eq!(request_messages(&chat, &off, "промпт").len(), 6);
    ...
    fn the_window_sends_only_the_last_messages() {
        ...
        assert_eq!(request_messages(&chat, &wide, "промпт").len(), 6);
        ...
        let messages = request_messages(&with_summary, &settings, "промпт");
        assert!(messages.iter().all(|m| !m.content.contains("было раньше")));
    }

    #[test]
    fn facts_ride_along_as_a_second_system_message() {
        let chat = Chat {
            facts: vec![
                Fact { key: "бюджет".to_string(), value: "400 тысяч рублей".to_string() },
                Fact { key: "срок".to_string(), value: "3 месяца".to_string() },
            ],
            ..chat_with_history()
        };
        let settings = Settings { strategy: Strategy::Facts, keep_last: 2, ..Settings::default() };

        let messages = request_messages(&chat, &settings, "промпт");
        assert_eq!(messages.len(), 4, "промпт, факты и два сообщения окна");
        assert_eq!(messages[1].role, "system", "факты — не чья-то реплика");
        assert_eq!(
            messages[1].content,
            "Известные факты о диалоге:\n- бюджет: 400 тысяч рублей\n- срок: 3 месяца"
        );
        assert_eq!(messages[2].content, "ответ два");
        assert_eq!(messages[3].content, "третий");

        // Пустая память блока не даёт: пустую шапку слать незачем.
        let empty = Chat { facts: Vec::new(), ..chat.clone() };
        let messages = request_messages(&empty, &settings, "промпт");
        assert_eq!(messages.len(), 3);
        assert!(messages.iter().all(|m| !m.content.starts_with("Известные факты")));

        // При другой стратегии факты из файла в запрос не идут.
        let off = Settings { strategy: Strategy::Full, ..settings };
        let messages = request_messages(&chat, &off, "промпт");
        assert!(messages.iter().all(|m| !m.content.starts_with("Известные факты")));
        assert_eq!(chat.facts.len(), 2, "смена стратегии факты не удаляет");
    }
    ...
    #[test]
    fn the_facts_request_is_short_and_is_not_streamed() {
        let long = "я".repeat(3000);
        // В значении нет буквы «я» — ниже по ней считается обрезка реплики.
        let facts = [Fact { key: "бюджет".to_string(), value: "400 тыс. рублей".to_string() }];

        let body = facts_body(Provider::DeepSeek, "deepseek-v4-flash", &facts, &long, "коротко");
        assert_eq!(body["stream"], json!(false));
        assert_eq!(body["temperature"], json!(0.2));
        assert_eq!(body["max_tokens"], json!(400));
        assert_eq!(body["thinking"]["type"], json!("disabled"), "рассуждение памяти не нужно");
        assert_eq!(body["messages"][0]["content"], json!(FACTS_PROMPT));
        // Промпт просит дельту и объясняет, чем удалить факт: на этом держится
        // слияние — повторять неизменившееся модель больше не обязана.
        assert!(FACTS_PROMPT.contains("которые появились или изменились"), "{FACTS_PROMPT}");
        assert!(FACTS_PROMPT.contains("пустой строкой"), "{FACTS_PROMPT}");

        let user = body["messages"][1]["content"].as_str().expect("вторая реплика — строка");
        assert!(user.starts_with("Текущие факты:\n{\"бюджет\":\"400 тыс. рублей\"}"));
        assert_eq!(user.matches('я').count(), FACTS_SOURCE_LIMIT, "длинная реплика обрезана");
        assert!(user.contains("\n\nАссистент: коротко"));

        // Без фактов модель получает пустой объект, а не пустую строку.
        let first = facts_body(Provider::Cerebras, "qwen-3.8-27b", &[], "вопрос", "ответ");
        assert_eq!(first["max_completion_tokens"], json!(400));
    ...
    #[test]
    fn facts_are_parsed_from_json_with_or_without_a_fence() {
        let facts = parse_facts(r#"{"бюджет":"400 тысяч","срок":"3 месяца"}"#).expect("чистый JSON");
        assert_eq!(facts.len(), 2);
        assert_eq!(facts[0], Fact { key: "бюджет".to_string(), value: "400 тысяч".to_string() });

        let fenced = parse_facts("```json\n{\"цель\": \"робот-курьер\"}\n```").expect("обёртка снята");
        assert_eq!(fenced, vec![Fact { key: "цель".to_string(), value: "робот-курьер".to_string() }]);

        // Числа и прочие не-строки становятся текстом, а не теряются.
        let mixed = parse_facts(r#"{"бюджет": 400000, "шум": true, " ": "пусто", "":"тоже"}"#)
        ...
        // Дубль по ключу без учёта регистра в память не попадает дважды.
        let dupes = parse_facts(r#"{"Бюджет":"400 тысяч","бюджет":"500 тысяч"}"#).unwrap();
        assert_eq!(dupes.len(), 1);

        // Всё, что не объект, — ошибка: прежние факты останутся на месте.
        assert!(parse_facts("конечно, вот факты: бюджет 400 тысяч").is_err());
        assert!(parse_facts("[1, 2, 3]").is_err());
        assert!(parse_facts("").is_err());

        // Пустой объект разбирается: это «без изменений», а не ошибка.
        assert_eq!(parse_facts("{}").unwrap(), Vec::new());

        // Пустое значение доживает до слияния — там оно забывает факт.
        let erase = parse_facts(r#"{"бюджет":"  "}"#).unwrap();
        assert_eq!(erase, vec![Fact { key: "бюджет".to_string(), value: String::new() }]);

        // Потолок памяти применяется при слиянии, разбор дельту не режет.
        let many: String = (0..60).map(|i| format!("\"к{i}\":\"з{i}\"")).collect::<Vec<_>>().join(",");
        assert_eq!(parse_facts(&format!("{{{many}}}")).unwrap().len(), 60);
    }
    ...
    fn merging_stops_adding_at_the_limit_but_keeps_what_is_stored() {
        let mut facts: Vec<Fact> = (0..FACTS_LIMIT).map(|i| fact(&format!("к{i}"), "старое")).collect();

        merge_facts(&mut facts, vec![fact("к0", "новое"), fact("лишний", "не влезет")]);
        assert_eq!(facts.len(), FACTS_LIMIT, "старые факты лимитом не выбрасываются");
        ...
        assert_eq!(facts.len(), FACTS_LIMIT);
        assert_eq!(facts[FACTS_LIMIT - 1], fact("лишний", "теперь влезет"));
    }
