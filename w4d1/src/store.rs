...
const DEFAULT_INSTRUCTIONS: &str = "Я исследую разные темы и сравниваю подходы. Помогай формулировать выводы по источникам, отделяя факты от предположений. Обращайся на «ты». Отвечай коротко и по делу, без воды; код — только когда он нужен. Если чего-то не знаешь или не уверен — так и скажи, не выдумывай.";

/// Начальные правила исследования; пользователь может изменить их в интерфейсе.
const DEMO_INVARIANTS: &[(Category, &str, &str)] = &[
    (
        Category::Decision,
        "Не выдумывать источники, ссылки, цитаты и результаты поиска",
        "вывод должен опираться на реально доступные материалы",
    ),
    (
        Category::Decision,
        "Явно отделять подтверждённые сведения от предположений и неизвестного",
        "уверенный тон не заменяет доказательство",
    ),
    (
        Category::Decision,
        "Не утверждать, что поиск или чтение страницы выполнены, если нет результата вызова инструмента",
        "каталог MCP показывает возможности сервера, но не результаты исследования",
    ),
    (
        Category::Business,
        "Не объявлять исследование проверенным, пока существенные выводы не сопоставлены с источниками",
        "отсутствие данных — пробел, а не успешная проверка",
    ),
];
...
mod tests {
    ...
    fn instructions_are_seeded_once_and_survive_a_restart() {
        ...
        assert!(store.instructions().starts_with("Я исследую разные темы и сравниваю подходы. Помогай формулировать выводы по источникам, отделяя факты от предположений."));
    ...
    fn a_fresh_folder_gets_four_demo_invariants_once() {
        ...
        assert_eq!(
            categories,
            [Category::Decision, Category::Decision, Category::Decision, Category::Business]
        );
        assert!(set.items[1].rule.contains("предположений"), "{:?}", set.items[1]);
