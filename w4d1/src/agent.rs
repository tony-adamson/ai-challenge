...
MCP в этой версии показывает только каталог инструментов: у тебя пока нет возможности вызвать поиск или прочитать страницу через Exa.
...
impl Agent {
    ...
    async fn guard(
        ...
        question: &str,
        tx: &mpsc::Sender<Event>,
    ) -> Result<(String, Metrics, Verdict, usize), String> {
        ...
        Ok((answer, metrics, verdict, chars))
    }
