//! Генератор задач с известным ответом (скопирован из w1d3: папки дней
//! самодостаточны). Параметры случайные, эталон считает сам Rust — так
//! точность моделей измеряется честно, а свежие числа не могли попасть
//! в обучающие данные ни одной из них.
...
/// последняя непустая строка. Разметка снимается — модели любят обрамлять
/// итог звёздочками (`**3**`) и долларами (`$3$`), а сравнивать надо число.
pub fn extract_answer(content: &str) -> String {
    const MARKER: &str = "ответ:";
    let clean = content.replace(['*', '#', '`', '$'], "").to_lowercase();
    if let Some(pos) = clean.rfind(MARKER) {
        let tail = &clean[pos + MARKER.len()..];
        let first_line = tail.lines().next().unwrap_or("");
        // «Ответ:» бывает и заголовком: сам итог тогда на следующих строках
        // (обычно в \[ \boxed{…} \]). Пустая строка после маркера — знак
        // взять весь остаток, а не решить, что ответа нет вовсе.
        return normalize(if normalize(first_line).is_empty() { tail } else { first_line });
    }
    // Строка без единой буквы и цифры итогом быть не может: у ответа,
    // закрытого «\]», содержательная строка — предыдущая.
    normalize(clean.lines().rev().find(|l| l.chars().any(char::is_alphanumeric)).unwrap_or(""))
}
...
mod tests {
    ...
    /// Ответы из живого прогона w1d5 на «сколько раз буква е», эталон 3.
    #[test]
    fn real_answers_from_a_live_run() {
        let cases = [
            "Посчитаю по буквам.\nСумма: $2 + 1 = 3$.\n\nОтвет: **3**",
            "Разбираю строку.\nИтак, всего 3 буквы «е».\n\nОтвет: 3.",
            "Считаю: е встречается на позициях 11, 12, 23 — всего 3 раза.\n\nОтвет: **3**",
            "3",
        ];
        for answer in cases {
            assert!(matches(&extract_answer(answer), "3"), "не засчитан верный ответ: {answer:?}");
        }
        // Другое число в том же формате верным быть не должно.
        assert!(!matches(&extract_answer("Ответ: **17**"), "3"));
        assert!(!matches(&extract_answer("Ответ: 17"), "7"));
        // «Ответ:» как заголовок, а сам итог ниже, в формуле — так ответил
        // deepseek-v4-flash в живом прогоне, и вердикт был «неверно».
        assert!(matches(&extract_answer("Сумма цифр:\n4 + 4 + 0 + 8 + 2 + 8 = 26\n\nОтвет:\n\\[\n\\boxed{26}\n\\]"), "26"));
        // Итог только в формуле, без маркера вовсе.
        assert!(matches(&extract_answer("Считаю…\n\\[\n\\boxed{16119}\n\\]"), "16119"));
        // Английский «Final Answer» маркером не считается, спасает откат
        // на последнюю содержательную строку — так ответила gemma-3-12b-it.
        assert!(matches(&extract_answer("…прошло 16119 дней.\n\nFinal Answer: The final answer is $\\boxed{16119}$"), "16119"));
        // Ответ словом в предложении и с предлогом.
        assert!(matches(&extract_answer("Это была среда."), "среда"));
        assert!(matches(&extract_answer("Рассуждение.\n\nОтвет: **в среду**"), "среда"));
        assert!(!matches(&extract_answer("Ответ: четверг"), "среда"));
    }
