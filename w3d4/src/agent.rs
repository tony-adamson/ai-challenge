...
//! запроса, как и память, и подставляется сразу за личностью. Состояние
...
use crate::store::{
    same_key, Chat, Entry, Fact, Invariant, Kind, LongTerm, Note, Profile, Stage, Task, Working,
};
...
impl Default for Layers {
    fn default() -> Self {
        Layers { invariants: true, profile: true, long_term: true, task: true, working: true }
    }
}
...
/// чего ждём и что на этом этапе можно. Блок есть всегда — этап есть всегда,
/// даже у пустой задачи; пустые «шаг» и «ожидаемое действие» пропускаются.
///
/// Правила этапов и таблица переходов лежат в коде (`Stage`), а сюда только
/// переписываются: промпт теряется при суммаризации и правится руками, код —
/// нет. Модель в этом блоке не решает, куда переходить, ей говорят, где она
/// находится и чего на этом этапе делать нельзя.
///
...
pub fn task_message(task: &Task, resumed: bool) -> Option<Message> {
    ...
    lines.push(
        "Работай строго в рамках текущего этапа и не перепрыгивай вперёд: если человек просит то, что относится к следующему этапу, скажи, на каком этапе задача и чего не хватает, чтобы его закрыть. Когда этап по смыслу закрыт, скажи об этом явно."
            .to_string(),
    );
    Some(Message::new("system", lines.join("\n")))
}

/// Сколько в блоке задачи заполненных пунктов: этап есть всегда, шаг и
/// ожидаемое действие — по факту. Число идёт в разбивку под ответом.
pub fn task_items(task: &Task) -> usize {
    1 + [&task.step, &task.expected].iter().filter(|v| !v.trim().is_empty()).count()
}
...
/// 3. профиль: как отвечать этому человеку и в каком порядке;
...
///
/// Инварианты стоят ещё раньше профиля: ограничения профиля — пожелания
/// человека к форме ответа, а инвариант — граница того, что вообще можно
/// предложить, и он должен перекрывать всё, что идёт после.
///
...
const MEMORY_PROMPT: &str = "Ты ведёшь память агента и раскладываешь новое по четырём корзинам. На входе рабочая память текущей задачи, её состояние, ключи того, что уже известно про пользователя и лежит в долговременной памяти, и последняя пара реплик. Верни ТОЛЬКО JSON-объект вида {\"working\": {\"ключ\": \"значение\"}, \"profile\": [{\"key\": \"...\", \"value\": \"...\"}], \"long_term\": [{\"kind\": \"decision|knowledge\", \"key\": \"...\", \"value\": \"...\"}], \"task\": {\"step\": \"...\", \"expected\": \"...\", \"transition\": null}}.
...
В working верни только то, что появилось или изменилось в последней паре реплик; неизменившееся повторять не нужно; чтобы забыть факт, верни его ключ с пустой строкой. В profile и long_term не повторяй то, что уже есть в списках известного. task заполняй всегда. step — что делается сейчас, одной фразой. expected — какое действие ожидается дальше и от кого: от человека (ответить на вопрос, подтвердить решение) или от агента (составить план, проверить список). transition — следующий этап, и только если текущий по смыслу закрыт; иначе null. Этапы: planning — собираем требования и ограничения; execution — принимаем решения по собранным требованиям; validation — сверяем результат с требованиями и ищем потерянное; done — задача закрыта. Через этап не перепрыгивай; вернуться назад можно, если всплыло новое требование.
Нового нет — верни {\"working\": {}, \"profile\": [], \"long_term\": []} и заполненный task. Не добавляй ничего, чего не было в диалоге. Ключи короткие, на русском, значения не длиннее 120 символов. Без пояснений и без markdown.";
...
fn memory_body(
    ...
    let mut body = json!({
        ...
        "messages": [
            ...
            { "role": "user", "content": memory_source(working, task, profile, long_term, user, assistant) },
        ],
        "stream": false,
        "temperature": 0.2,
    });
...
/// Что служебный вызов сказал про задачу: шаг, ожидаемое действие и,
/// возможно, предложение следующего этапа. Само по себе это ещё не состояние:
/// переход проверяет код (`Task::transition`), и запрещённый он не применит.
...
fn parse_memory(
    raw: &str,
) -> Result<(Vec<Fact>, Vec<Fact>, Vec<Suggestion>, TaskUpdate), String> {
    ...
    Ok((working, profile, long_term, task))
}
...
/// ничего не меняет. Переход она только предлагает: разрешённый применяется,
/// запрещённый остаётся в журнале с пометкой, и этап стоит на месте. В этом и
/// смысл дня: таблица переходов живёт в коде, а не в промпте.
...
const VALIDATOR_PROMPT: &str = "Ты проверяешь ответ ассистента на соответствие инвариантам проекта. На входе список инвариантов, запрос пользователя и ответ ассистента. Верни ТОЛЬКО JSON-объект вида {\"violations\": [{\"id\": \"I2\", \"quote\": \"дословный фрагмент ответа\", \"why\": \"чем он противоречит инварианту\"}]}.
Нарушение — только когда ответ ПРЕДЛАГАЕТ, рекомендует, выбирает или даёт инструкцию, противоречащую инварианту: код, схему, список покупок, шаги, «как вариант» или «если очень хочется» — тоже нарушение.
НЕ нарушение: упоминание запрещённого варианта в отказе или в объяснении, почему он не подходит (например, «Raspberry Pi не подходит по I2»); первая строка «Сверка: …»; пересказ запроса пользователя; ответ, который инварианта не касается.
...
/// Вход валидатора: только включённые инварианты, последний запрос и ответ.
/// Историю разговора ему не даём — он судит один ответ, а не беседу.
fn validator_source(invariants: &[Invariant], question: &str, answer: &str) -> String {
    let list: Vec<String> = invariants
        .iter()
        .filter(|i| i.enabled)
        .map(|i| format!("{} [{}] {}", i.id, i.category.label(), i.rule.trim()))
        .collect();
    format!(
        "Инварианты:\n{}\n\nЗапрос пользователя:\n{}\n\nОтвет ассистента:\n{}",
        list.join("\n"),
...
fn validator_body(
    ...
    let mut body = json!({
        ...
        "messages": [
            ...
            { "role": "user", "content": validator_source(invariants, question, answer) },
        ],
        "stream": false,
        "temperature": 0.0,
    });
...
/// Реплика с разбором для перегенерации. Текст нарушенного инварианта
/// повторяется здесь же: слой инвариантов в запросе мог быть выключен, и без
/// этого модель не знала бы, что именно нарушила. Шаблон отказа — по той же
/// причине.
pub fn retry_message(violations: &[Violation], invariants: &[Invariant]) -> String {
    let lines: Vec<String> = violations
        .iter()
        .map(|v| {
            let rule = invariants.iter().find(|i| i.id == v.id).map_or("", |i| i.rule.trim());
            format!("- {} ({rule}) — «{}» — {}", v.id, v.quote.trim(), v.why.trim())
        })
        .collect();
    format!(
        "Твой ответ нарушил инварианты проекта:\n{}\nПерепиши ответ, соблюдая все инварианты. Нарушающее решение не предлагай даже как вариант. Если запрос нельзя выполнить в их рамках — откажи по шаблону: какой инвариант (id и текст) → почему запрос с ним конфликтует → что можно сделать в его рамках.",
        lines.join("\n")
    )
}
...
impl Agent {
    ...
    /// игнорируется.
    ///
    /// Есть включённые инварианты — текст наружу не стримится: сначала его
    /// проверяет валидатор (`guard`), и человек видит только проверенный
    /// ответ. Пока идёт проверка, уходят события `Phase` для лоадера.
    pub async fn ask(
        &self,
        chat: &mut Chat,
        invariants: &[Invariant],
        ...
        // Валидатор работает при любом положении тумблера слоя: выключенный
        // слой — ровно тот случай, когда вторая линия защиты и нужна.
        let guarded: Vec<Invariant> = invariants.iter().filter(|i| i.enabled).cloned().collect();
        let outcome = if guarded.is_empty() {
            self.stream(provider, &key, &chat.settings, &messages, Some(tx))
                .await
                .map(|(answer, _, metrics)| (answer, metrics, None, sent_chars))
        } else {
            self.guard(provider, &key, &chat.settings, &messages, &guarded, &text, tx).await
        };

        match outcome {
            Ok((answer, mut metrics, verdict, sent_chars)) => {
    ...
    /// Ответ под охраной инвариантов: черновик → проверка → при нарушении одна
    /// перегенерация → проверка. Текст отдаётся наружу только в конце, одним
    ...
    async fn guard(
        ...
        question: &str,
        tx: &mpsc::Sender<Event>,
    ) -> Result<(String, Metrics, Option<Verdict>, usize), String> {
        ...
        let (draft, reasoning, metrics) = self.stream(provider, key, settings, messages, None).await?;
        let _ = tx.send(phase("check", Vec::new())).await;
        let first = self.check(provider, key, &settings.model, invariants, question, &draft, &mut usage).await;
        ...
        match next_step(false, &first) {
            ...
            Next::Retry => {
                ...
                again.push(Message::new("user", retry_message(&first.violations, invariants)));
                match self.stream(provider, key, settings, &again, None).await {
                    Ok((answer, reasoning, metrics)) => {
                        let _ = tx.send(phase("check", Vec::new())).await;
                        let second =
                            self.check(provider, key, &settings.model, invariants, question, &answer, &mut usage).await;
        ...
        Ok((answer, metrics, Some(verdict), chars))
    }
    ...
    async fn check(
        ...
        let body = validator_body(provider, model, invariants, question, answer);
        ...
        let known: Vec<String> = invariants.iter().map(|i| i.id.clone()).collect();
    ...
    pub async fn update_memory(
        ...
        let (facts, profile, suggestions, task) = parse_memory(&text)?;
...
mod tests {
    ...
    fn the_full_strategy_sends_the_whole_history() {
        ...
        let messages = request_messages(&chat, &settings, "промпт", &[], &Profile::default(), &[], &Task::default(), &[]);
        ...
        assert_eq!(
            request_messages(&chat, &waiting, "промпт", &[], &Profile::default(), &[], &Task::default(), &[])
                .len(),
            6
        );
        let empty = Chat { summary: Some("   ".to_string()), summary_covers: 3, ..chat.clone() };
        assert_eq!(
            request_messages(&empty, &waiting, "промпт", &[], &Profile::default(), &[], &Task::default(), &[]).len(),
            6,
    ...
    /// этому человеку, что решено на все разговоры, где сейчас задача, факты
    ...
    fn the_layers_go_into_the_request_in_order() {
        ...
        let messages =
            request_messages(&chat, &settings, "промпт", &[], &profile, &long_term, &task, &working);
    ...
    fn a_switched_off_layer_disappears_from_the_request_only() {
        ...
        let without_profile = Settings {
            layers: Layers { invariants: true, profile: false, long_term: true, task: true, working: true },
            ..Settings::default()
        };
        let messages =
            request_messages(&chat, &without_profile, "промпт", &[], &profile, &long_term, &task, &working);
        ...
        let without_long = Settings {
            layers: Layers { invariants: true, profile: true, long_term: false, task: true, working: true },
            ..Settings::default()
        };
        let messages =
            request_messages(&chat, &without_long, "промпт", &[], &profile, &long_term, &task, &working);
        ...
        let without_task = Settings {
            layers: Layers { invariants: true, profile: true, long_term: true, task: false, working: true },
            ..Settings::default()
        };
        let messages =
            request_messages(&chat, &without_task, "промпт", &[], &profile, &long_term, &task, &working);
        ...
        let without_working = Settings {
            layers: Layers { invariants: true, profile: true, long_term: true, task: true, working: false },
            ..Settings::default()
        };
        let messages =
            request_messages(&chat, &without_working, "промпт", &[], &profile, &long_term, &task, &working);
        ...
        let neither = Settings {
            layers: Layers { invariants: true, profile: false, long_term: false, task: false, working: false },
            ..Settings::default()
        };
        let messages =
            request_messages(&chat, &neither, "промпт", &[], &profile, &long_term, &task, &working);
    ...
    fn layer_stats_count_what_actually_went_out() {
        ...
        let stats = layer_stats(&chat, &settings, &[], &profile, &long_term, &Task::default(), &working);
        ...
        let off = Settings {
            layers: Layers { invariants: true, profile: false, long_term: false, task: false, working: true },
            ..settings.clone()
        };
        let stats = layer_stats(&chat, &off, &[], &profile, &long_term, &Task::default(), &working);
        ...
        let stats = layer_stats(&chat, &settings, &[], &Profile::default(), &[], &Task::default(), &[]);
    ...
    fn a_summary_replaces_the_covered_head_of_the_history() {
        ...
        let messages =
            request_messages(&chat, &settings, "промпт", &[], &Profile::default(), &[], &Task::default(), &[]);
        ...
        assert_eq!(
            request_messages(&chat, &off, "промпт", &[], &Profile::default(), &[], &Task::default(), &[])
                .len(),
            6
        );
    ...
    fn the_window_sends_only_the_last_messages() {
        ...
        assert_eq!(
            request_messages(&chat, &wide, "промпт", &[], &Profile::default(), &[], &Task::default(), &[])
                .len(),
            6
        );
    ...
    #[test]
    fn the_memory_reply_is_split_into_four_baskets() {
        let (working, profile, long_term, _) = parse_memory(
        ...
        assert_eq!(fenced.3, TaskUpdate::default(), "корзины задачи нет — состояние не трогаем");
        ...
        assert_eq!(
            ...
            (vec![], vec![], vec![], TaskUpdate::default())
        );
        assert_eq!(parse_memory("{}").unwrap(), (vec![], vec![], vec![], TaskUpdate::default()));

        // Пустое значение в working доживает до слияния — там оно забывает факт.
        let (erase, _, _, _) = parse_memory(r#"{"working":{"бюджет":"  "}}"#).unwrap();
    ...
    fn the_task_block_says_where_the_task_is_and_what_is_allowed() {
        ...
        assert!(filled.contains("Правило этапа: принимай решения"), "{filled}");
    ...
    fn the_agent_proposes_a_transition_and_the_code_decides() {
        ...
        // Разрешённый переход применяется.
        apply_task(
            &TaskUpdate { transition: Some(Stage::Execution), ..TaskUpdate::default() },
            &mut task,
        );
        ...
        assert_eq!(task.log[1].note, crate::store::REJECTED_NOTE);
        assert_eq!(task.log[1].to, Stage::Done);
    ...
    fn applying_memory_moves_the_task_too() {
        ...
        let reply = MemoryReply {
            ...
            task: TaskUpdate {
                step: "собираем требования".to_string(),
                expected: "человек отвечает на вопрос".to_string(),
                transition: Some(Stage::Execution),
            },
            profile: vec![],
            suggestions: vec![],
            prompt_tokens: 100,
        ...
        let info = apply_memory(reply, &mut profile, &mut long_term, &mut working);
        assert_eq!(working.facts.len(), 1);
        assert_eq!(working.task.stage, Stage::Execution);
        assert_eq!(info.task.stage, Stage::Execution, "странице уезжает новое состояние");
    ...
    /// Инварианты встают сразу за личностью, до профиля; выключенный слой
    ...
    fn the_invariants_layer_goes_first_and_is_counted() {
        ...
        assert!(messages[2].content.starts_with("Профиль пользователя"));

        let stats = layer_stats(&chat, &settings, &invariants, &Profile::default(), &[], &Task::default(), &[]);
        ...
        let messages =
            request_messages(&chat, &off, "промпт", &invariants, &Profile::default(), &[], &Task::default(), &[]);
        assert!(messages.iter().all(|m| !m.content.starts_with("Инварианты")));
        let stats = layer_stats(&chat, &off, &invariants, &Profile::default(), &[], &Task::default(), &[]);
    ...
    fn the_validator_request_is_strict_and_short() {
        let invariants = test_invariants();
        let body = validator_body(Provider::Cerebras, "qwen-3.8-27b", &invariants, "вопрос", "ответ");
        ...
        let oss = validator_body(Provider::Cerebras, "gpt-oss-120b", &invariants, "в", "о");
        assert_eq!(oss["reasoning_effort"], "low");
        let deepseek = validator_body(Provider::DeepSeek, "deepseek-v4-flash", &invariants, "в", "о");
    ...
    fn the_verdict_decides_between_accept_and_one_retry() {
        ...
        // Разбор для перегенерации повторяет текст нарушенного инварианта:
        // слой в запросе мог быть выключен.
        let text = retry_message(&dirty.violations, &test_invariants());
        assert!(text.contains("- I2 (Контроллер — ESP32-C3) — «q» — w"), "{text}");
