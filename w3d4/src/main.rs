//! w3d4 — веб-чат с явной моделью памяти агента, профилем пользователя,
//! состоянием задачи как конечным автоматом и инвариантами проекта.
//! Здесь только HTTP: маршруты, общее состояние и переклад событий агента в
//! SSE. Слои памяти, профили и инварианты лежат в файлах (`store.rs`), формат
...
use agent::{
    invariants_message, is_context_overflow, persona, Agent, Event, Provider, Settings, Strategy,
    PERSONAS,
};
use store::{Category, Chat, Entry, Invariants, Kind, LongTerm, Profile, Stage, Store, Working, DATA_DIR};

const ADDR: &str = "127.0.0.1:8798";

struct App {
    agent: Agent,
    store: Store,
    /// Замок на правку памяти, профиля и инвариантов. Все они правятся схемой «прочитать
...
async fn start() -> Result<(), String> {
    println!("AI Advent · w3d4 — инварианты");
...
async fn state(State(app): State<Arc<App>>) -> Json<Value> {
    ...
    Json(json!({
        ...
        "strategies": Strategy::ALL.iter()
            ...
            .collect::<Vec<_>>(),
        // Этапы задачи и разрешённые переходы — из кода, а не из копии на
        // странице: в этом весь смысл автомата. Панель рисует по этому списку
        // полоску этапов и решает, какие кнопки активны.
...
/// Ручной перевод этапа. Таблица та же, что у предложений агента
/// (`Stage::allowed`), меняется только подпись в журнале: `human` вместо
/// `agent`. Запрещённый переход — 400 с текстом и ничего на диске: кнопка
/// такого этапа в панели и так недоступна, а мимо неё сюда приходят разве что
/// curl-ом.
async fn task_transition(
    ...
    let _guard = app.memory_lock.lock().await;
    let mut working = app.store.working(&id);
    if let Err(error) = working.task.transition(to, "human", "кнопка в панели") {
        return (StatusCode::BAD_REQUEST, error).into_response();
    }
    save_working(&app, &id, &working)
}

/// Пауза: этап, шаг и ожидаемое действие остаются на диске как есть. Запросы
/// она не блокирует — это отметка «мы вышли», и весь смысл в том, что после
...
async fn ask(
    ...
    tokio::spawn(async move {
        ...
        let event =
            match app
                .agent
                .ask(&mut chat, &invariants.items, &profile, &long_term, &working, &request.text, &tx)
                .await
            {
        ...
        // Флаг «только что продолжили после паузы» действует ровно на один
        // запрос — он уже ушёл. Снимаем до обновления памяти: оно перечитает
        // рабочую с диска и вернёт странице состояние задачи уже без флага.
        if answered {
            let _guard = app.memory_lock.lock().await;
            let mut working = app.store.working(&chat.id);
            if working.task.resumed_from.is_some() {
                working.task.resumed_from = None;
