...
//!   действие и журнал переходов — чат и есть задача;
...
//! `Working`, `LongTerm`, `Profile`, `Invariants` и `Store`.
//!
...
/// Папка данных относительно запуска (`cargo run` из `w3d4`).
...
impl Stage {
    ...
    pub fn rule(&self) -> &'static str {
        match self {
            Stage::Planning => "собирай требования и ограничения, задавай уточняющие вопросы по одному, решений пока не предлагай",
            Stage::Execution => "принимай решения и предлагай варианты по уже собранным требованиям, не выдумывая новых ограничений",
            Stage::Validation => "сверяй результат с требованиями и ограничениями, ищи потерянное и противоречия, ничего нового не добавляй",
...
impl Task {
    /// Перевод этапа по таблице `Stage::allowed`. Запрещённый переход этап не
    /// меняет, но в журнал попадает с пометкой: и предложение агента, и
    /// нажатие человека мимо интерфейса должны оставлять след.
    ...
    /// Пауза: этап и контекст остаются на диске как есть. Запросы она не
    /// блокирует — это отметка «мы вышли», а не замок.
...
mod tests {
    ...
    fn temp_store(name: &str) -> (PathBuf, Store) {
        let dir = std::env::temp_dir().join(format!("w3d4-{name}-{}-{}", std::process::id(), new_id()));
    ...
    fn a_task_survives_a_round_trip_through_a_file() {
        ...
        working.task.transition(Stage::Execution, "agent", "требования собраны").unwrap();
        ...
        assert_eq!(back.task.stage, Stage::Execution);
