//! w3d1 — веб-чат с явной моделью памяти агента. Здесь только HTTP: маршруты,
//! общее состояние и переклад событий агента в SSE. Три слоя памяти лежат в
//! трёх файлах (`store.rs`), формат API провайдеров, сборка запроса по слоям и
//! вся арифметика токенов — в `agent.rs`.
...
use store::{Chat, Entry, Kind, LongTerm, Store, Working, DATA_DIR};
...
struct App {
    agent: Agent,
    store: Store,
    /// Замок на правку памяти. Оба слоя правятся схемой «прочитать файл —
    /// изменить — записать», и `long_term.json` к тому же общий на все чаты:
    /// без сериализации второй писатель молча затирает первого. Замок один на
    /// оба слоя, потому что «→ долговременная» и обновление после ответа
    /// трогают их вместе.
...
/// В какой тип переводить факт рабочей памяти.
#[derive(Deserialize)]
struct KindRequest {
    kind: String,
}
...
async fn start() -> Result<(), String> {
    println!("AI Advent · w3d1 — агент с моделью памяти");
...
async fn ask(
    ...
    tokio::spawn(async move {
        ...
        let event = match app.agent.ask(&mut chat, &long_term, &working, &request.text, &tx).await {
