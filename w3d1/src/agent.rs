...
//! потока. Память агент не хранит — три её слоя лежат в трёх файлах (см.
//! `store.rs`), агент их только читает и дописывает. Про HTTP-сервер здесь не
//! знают: наружу торчат `Provider`, `Settings`, `Message`, `Metrics` и поток
//! `Event`.
...
use crate::store::{same_key, Chat, Entry, Fact, Kind, LongTerm, Working};
...
impl Default for Layers {
    fn default() -> Self {
        Layers { long_term: true, working: true }
    }
}
...
/// 2. долговременная память: кто человек и что решено на все разговоры;
/// 3. рабочая память: факты текущей задачи;
/// 4. краткосрочная память: сам диалог в форме, которую задала стратегия.
///
...
const MEMORY_PROMPT: &str = "Ты ведёшь память агента и раскладываешь новое по двум слоям. На входе рабочая память текущей задачи, список того, что уже лежит в долговременной памяти, и последняя пара реплик. Верни ТОЛЬКО JSON-объект вида {\"working\": {\"ключ\": \"значение\"}, \"long_term\": [{\"kind\": \"profile|decision|knowledge\", \"key\": \"...\", \"value\": \"...\"}]}.
Куда что класть. working — всё, что относится только к текущей задаче или разговору: требования, цифры, выбранные варианты, открытые вопросы. long_term с kind=profile — факты о самом человеке: роль, уровень, как ему отвечать, какие инструменты он предпочитает. long_term с kind=decision — решения, которые человек явно назвал общими для всех проектов или разговоров. long_term с kind=knowledge — проверенные знания, которые человек попросил запомнить навсегда. Сомневаешься — клади в working.
В working верни только то, что появилось или изменилось в последней паре реплик; неизменившееся повторять не нужно; чтобы забыть факт, верни его ключ с пустой строкой. В long_term не повторяй то, что уже есть в списке известного. Нового нет — верни {\"working\": {}, \"long_term\": []}. Не добавляй ничего, чего не было в диалоге. Ключи короткие, на русском, значения не длиннее 120 символов. Без пояснений и без markdown.";
...
/// типами и ключами. Значения оттуда в этот вызов не нужны, а дубли по ключу
/// модель должна видеть, иначе она предложит то же самое второй раз.
fn memory_source(working: &[Fact], long_term: &[Entry], user: &str, assistant: &str) -> String {
...
fn memory_body(
    ...
    let mut body = json!({
        ...
        "messages": [
            ...
            { "role": "user", "content": memory_source(working, long_term, user, assistant) },
        ],
        "stream": false,
        "temperature": 0.2,
    });
...
/// Разбор ответа хранителя памяти: две корзины сразу. Всё, что не разобралось
/// как объект, — ошибка: лучше оставить оба слоя прежними, чем испортить их.
/// Элемент `long_term` с неизвестным `kind` или без ключа отбрасывается молча —
/// один кривой элемент не повод терять остальные.
fn parse_memory(raw: &str) -> Result<(Vec<Fact>, Vec<Suggestion>), String> {
...
impl Agent {
    ...
    pub async fn ask(
        ...
        let layers = layer_stats(chat, &chat.settings, &long_term.entries, &working.facts);
    ...
    pub async fn update_memory(
        ...
        let (facts, suggestions) = parse_memory(&text)?;
...
mod tests {
    ...
    fn the_full_strategy_sends_the_whole_history() {
        ...
        assert_eq!(
            request_messages(&empty, &waiting, "промпт", &[], &[]).len(),
            6,
            "пустой пересказ не в счёт"
        );
    }

    /// Порядок слоёв в запросе — и есть модель памяти: личность, кто человек,
    /// что за задача, сам диалог.
    #[test]
    fn the_layers_go_into_the_request_in_order() {
        let chat = chat_with_history();
        let settings = Settings { keep_last: 2, ..Settings::default() };
        let long_term = [
            entry(Kind::Profile, "роль", "студент-робототехник"),
            entry(Kind::Knowledge, "порог датчика", "17 см"),
            entry(Kind::Profile, "стиль", "коротко, с формулами"),
        ];
        let working = [fact("бюджет", "400 тысяч"), fact("срок", "3 месяца")];

        let messages = request_messages(&chat, &settings, "промпт", &long_term, &working);
        assert_eq!(messages.len(), 8, "промпт, два слоя и пять реплик: {messages:?}");
        assert_eq!(messages[0].content, "промпт");
        assert_eq!(messages[1].role, "system", "долговременная — не чья-то реплика");
        // Записи одного типа стоят вместе, типы — в порядке Kind::ALL.
        assert_eq!(
            messages[1].content,
            "Долговременная память (о пользователе и общие решения):\n\
             Профиль:\n- роль: студент-робототехник\n- стиль: коротко, с формулами\n\
             Знания:\n- порог датчика: 17 см"
        );
        ...
        assert_eq!(messages[3].content, "первый", "дальше идёт сам диалог");
        assert_eq!(messages.last().map(|m| m.content.as_str()), Some("третий"));
    }
    ...
    fn a_switched_off_layer_disappears_from_the_request_only() {
        let chat = chat_with_history();
        let long_term = [entry(Kind::Profile, "роль", "студент-робототехник")];
        let working = [fact("бюджет", "400 тысяч")];

        let without_long = Settings {
            layers: Layers { long_term: false, working: true },
            ..Settings::default()
        };
        let messages = request_messages(&chat, &without_long, "промпт", &long_term, &working);
        ...
        let without_working = Settings {
            layers: Layers { long_term: true, working: false },
            ..Settings::default()
        };
        let messages = request_messages(&chat, &without_working, "промпт", &long_term, &working);
        ...
        let neither =
            Settings { layers: Layers { long_term: false, working: false }, ..Settings::default() };
        let messages = request_messages(&chat, &neither, "промпт", &long_term, &working);
        assert_eq!(messages.len(), 6, "остались промпт и история");
        assert_eq!(messages[1].content, "первый");

        // Включённый, но пустой слой блока тоже не даёт: пустую шапку модели
        // слать незачем.
        let messages = request_messages(&chat, &Settings::default(), "промпт", &[], &[]);
    ...
    fn layer_stats_count_what_actually_went_out() {
        ...
        let long_term = [entry(Kind::Profile, "роль", "студент")];
        let working = [fact("бюджет", "400 тысяч")];

        // Ожидания — посчитанные руками числа, а не те же функции ещё раз:
        // «Долговременная память (о пользователе и общие решения):\nПрофиль:\n
        // - роль: студент» — 80 символов, при четырёх символах на токен это 20.
        let stats = layer_stats(&chat, &settings, &long_term, &working);
        ...
        let off =
            Settings { layers: Layers { long_term: false, working: true }, ..settings.clone() };
        let stats = layer_stats(&chat, &off, &long_term, &working);
        ...
        let stats = layer_stats(&chat, &settings, &[], &[]);
    ...
    fn a_summary_replaces_the_covered_head_of_the_history() {
        ...
        let layered = request_messages(
            ...
            &settings,
            "промпт",
            &[entry(Kind::Profile, "роль", "студент")],
            &[fact("бюджет", "400 тысяч")],
        );
        assert!(layered[1].content.starts_with("Долговременная память"));
        assert!(layered[2].content.starts_with("Рабочая память"));
        assert!(layered[3].content.starts_with("Краткое содержание"));
    ...
    fn the_window_sends_only_the_last_messages() {
        ...
        let working = [fact("бюджет", "400 тысяч")];
        let messages = request_messages(&chat, &settings, "промпт", &[], &working);
        assert_eq!(messages[1].content, "Рабочая память задачи:\n- бюджет: 400 тысяч");
    ...
    fn the_memory_request_is_short_and_is_not_streamed() {
        ...
        for rule in ["kind=profile", "kind=decision", "kind=knowledge", "клади в working"] {
        ...
        // Долговременная уходит типами и ключами: значения в этом вызове не
        // нужны, а дубли модель должна видеть.
        assert!(user.contains("Уже в долговременной памяти:\n- profile / роль"), "{user}");
        assert!(!user.contains("студент-робототехник"), "значения записей не отправляем");
        ...
        let first = memory_body(Provider::Cerebras, "qwen-3.8-27b", &[], &[], "вопрос", "ответ");
    ...
    #[test]
    fn the_memory_reply_is_split_into_two_baskets() {
        let (working, long_term) = parse_memory(
            r#"{"working":{"бюджет":"400 тысяч","срок":"3 месяца"},
                "long_term":[{"kind":"profile","key":"роль","value":"студент-робототехник"},
                             {"kind":"decision","key":"язык ответов","value":"русский"}]}"#,
        )
        .expect("чистый JSON");
        assert_eq!(working.len(), 2);
        assert_eq!(working[0], fact("бюджет", "400 тысяч"));
        assert_eq!(long_term.len(), 2);
        assert_eq!(long_term[0].kind, Kind::Profile);
        assert_eq!(long_term[0].key, "роль");
        assert_eq!(long_term[1].kind, Kind::Decision);
        ...
        // один кривой элемент не повод терять остальные.
        let (_, kinds) = parse_memory(
            r#"{"long_term":[{"kind":"телепатия","key":"а","value":"б"},
                             {"kind":"profile","key":"  ","value":"б"},
                             {"kind":"profile","key":"роль","value":""},
        ...
        let (_, dupes) = parse_memory(
            r#"{"long_term":[{"kind":"profile","key":"Роль","value":"студент"},
                             {"kind":"profile","key":"роль","value":"инженер"},
                             {"kind":"decision","key":"роль","value":"другой тип"}]}"#,
        )
        .unwrap();
        ...
        // Всё, что не объект, — ошибка: оба слоя останутся прежними.
        ...
        assert_eq!(parse_memory(r#"{"working":{},"long_term":[]}"#).unwrap(), (vec![], vec![]));
        assert_eq!(parse_memory("{}").unwrap(), (vec![], vec![]));

        // Пустое значение в working доживает до слияния — там оно забывает факт.
        let (erase, _) = parse_memory(r#"{"working":{"бюджет":"  "}}"#).unwrap();
    ...
    fn suggestions_wait_in_the_queue_and_duplicates_are_dropped() {
        ...
        long_term.entries.push(entry(Kind::Profile, "роль", "студент-робототехник"));
        ...
        let added = add_pending(
            ...
            vec![
                ...
                suggest(Kind::Profile, "Роль", "инженер"),
