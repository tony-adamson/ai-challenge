...
//! таблица в коде, а не в промпте. Про HTTP-сервер
...
use crate::store::{same_key, Chat, Entry, Fact, Kind, LongTerm, Note, Profile, Stage, Task, Working};
...
/// 4. состояние задачи: этап, шаг, ожидаемое действие и правило этапа;
/// 5. рабочая память: факты текущей задачи;
/// 6. краткосрочная память: сам диалог в форме, которую задала стратегия.
///
...
mod tests {
    ...
    fn a_switched_off_layer_disappears_from_the_request_only() {
        ...
        let without_profile = Settings {
            layers: Layers { profile: false, long_term: true, task: true, working: true },
            ..Settings::default()
        };
        let messages =
        ...
        let without_long = Settings {
            layers: Layers { profile: true, long_term: false, task: true, working: true },
            ..Settings::default()
        };
        let messages =
        ...
        let without_task = Settings {
            layers: Layers { profile: true, long_term: true, task: false, working: true },
            ..Settings::default()
        };
        let messages =
        ...
        let without_working = Settings {
            layers: Layers { profile: true, long_term: true, task: true, working: false },
            ..Settings::default()
        };
        let messages =
        ...
        let neither = Settings {
            layers: Layers { profile: false, long_term: false, task: false, working: false },
            ..Settings::default()
        };
        let messages =
    ...
    fn layer_stats_count_what_actually_went_out() {
        ...
        let off = Settings {
            layers: Layers { profile: false, long_term: false, task: false, working: true },
            ..settings.clone()
        };
