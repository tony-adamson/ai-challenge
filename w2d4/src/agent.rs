...
pub struct Settings {
    ...
    /// Сжимать ли историю самим агентом: начало разговора уезжает в summary,
    /// хвост уходит как есть. Выключение summary из файла не удаляет — его
    /// просто перестают подставлять в запрос.
    #[serde(default)]
    pub compress: bool,
    /// Сколько последних сообщений всегда идут в запрос как есть.
...
pub struct Metrics {
    ...
    /// только когда сжатие реально применилось: пара «факт `prompt_tokens` /
    /// эта оценка» и есть «сэкономлено».
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Metrics {
    fn build(
        ...
        Metrics {
            ...
            // Про сжатие знает не стрим, а `ask`: он собирал запрос и он же
...
/// Идёт ли в этот запрос summary вместо начала истории. Выключенное сжатие
/// пересказ из файла не трогает — просто не подставляет его.
fn uses_summary(chat: &Chat, settings: &Settings) -> bool {
    settings.compress && chat.summary.as_deref().is_some_and(|s| !s.trim().is_empty())
}
...
/// Что уходит провайдеру. Без сжатия — системный промпт и вся история; со
/// сжатием и готовым пересказом — промпт, пересказ и всё непокрытое им (между
/// суммаризациями хвост длиннее `keep_last`, так и задумано). Записи `error`
/// отбрасываются здесь же: в API такой роли нет.
///
/// Единственное место сборки запроса — по нему же считаются `sent_chars` для
/// калибровки, поэтому калибровка остаётся честной и под сжатием.
pub fn request_messages(chat: &Chat, settings: &Settings, system_prompt: &str) -> Vec<Message> {
    ...
    let tail = if uses_summary(chat, settings) {
...
impl Agent {
    ...
    pub async fn ask(
        ...
        // Символы считаем по тому, что реально уходит (вместе с сообщением
        // summary): по ним и по `prompt_tokens` из ответа калибруется
        // «сколько символов в токене» для оценки на клиенте.
        let sent_chars = sent_chars(&messages);
        let compressed = uses_summary(chat, &chat.settings);
        // Первое сообщение — системный промпт, второе под сжатием — summary:
        // реплики разговора начинаются после них.
        let sent_messages = messages.len() - 1 - usize::from(compressed);
        let summary_estimate = compressed.then(|| {
            estimate_tokens(messages[1].content.chars().count(), chat.chars_per_token_or_default())
        });
        let full_estimate =
            compressed.then(|| full_history_estimate(chat, &chat.settings.system_prompt));
...
mod tests {
    ...
    #[test]
    fn without_compression_the_whole_history_goes_into_the_request() {
        ...
        // Сжатие включено, а пересказа ещё нет — история идёт целиком.
        let waiting = Settings { compress: true, ..Settings::default() };
    ...
    fn a_summary_replaces_the_covered_head_of_the_history() {
        ...
        let settings = Settings { compress: true, keep_last: 2, ..Settings::default() };
        ...
        // Выключили сжатие — пересказ остался в файле, но в запрос не идёт.
        let off = Settings { compress: false, ..settings };
    ...
    fn compression_limits_are_validated() {
        let ok = Settings { compress: true, keep_last: 6, summarize_every: 10, ..Settings::default() };
        assert!(ok.validate().is_ok());
