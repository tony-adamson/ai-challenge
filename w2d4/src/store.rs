...
/// Папка данных относительно запуска (`cargo run` из `w2d4`).
...
mod tests {
    ...
    fn temp_store(name: &str) -> (PathBuf, Store) {
        let dir = std::env::temp_dir().join(format!("w2d4-{name}-{}-{}", std::process::id(), new_id()));
    ...
    fn compressing(keep_last: usize, summarize_every: usize) -> Settings {
        Settings { compress: true, keep_last, summarize_every, ..Settings::default() }
    }
    ...
    fn the_summary_threshold_counts_uncovered_messages() {
        ...
        // Выключенное сжатие порогов не имеет.
        let off = Settings { compress: false, ..settings.clone() };
    ...
    fn old_chats_without_summary_fields_still_load() {
        ...
        assert!(!chat.settings.compress, "старый чат сжатия не просил");
